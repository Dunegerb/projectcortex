...
## Animação de entrada 1.2.8

A abertura usa diretamente o HTML/CSS aprovado em `CortexApp/Resources/SplashIntro.html`, carregado localmente por `WKWebView`. Dessa forma, o IPA preserva a composição 739 × 1600, o recorte do `glasslogo`, os filtros SVG, a refração, o blur de 23,4 px, o hold inicial de 900 ms e a transição de 800 ms com `cubic-bezier(1, .01, 0, .99)`. O Swift encerra o loader apenas após o `transitionend` real. O Launch Screen usa um recorte renderizado do frame 1 para manter continuidade visual.
